...
void setSpeed(int towards, int PWM_Setl, int PWM_Setr){
    PWM_Setl = PWM_Setl < 65 ? PWM_Setl : 65;        // Since we are using 2 different motors, the PWM must be no more than 65.      
    PWM_Setr = PWM_Setr < 65 ? PWM_Setr : 65;
    
    if (PWM_Setl <= 100 && PWM_Setr <= 100){
        PCA9685_SetOutput(PCA_ADDRESS, LEFT1_TIRE_ENA , 0, 0xfff*PWM_Setl/100);
        PCA9685_SetOutput(PCA_ADDRESS, LEFT2_TIRE_ENA , 0, 0xfff*(PWM_Setl+35)/100);  // Our motors may have sth. wrong, so I add 10% pwm on them.
        PCA9685_SetOutput(PCA_ADDRESS, RIGHT1_TIRE_ENA, 0, 0xfff*PWM_Setr/100);
        PCA9685_SetOutput(PCA_ADDRESS, RIGHT2_TIRE_ENA, 0, 0xfff*(PWM_Setr+35)/100);
    }
    switch (towards) {
